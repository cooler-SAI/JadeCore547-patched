    ...
    bool Execute(uint64 /*currTime*/, uint32 /*diff*/)
    {
        if (obj)
        {
            if (InstanceScript* l_Instance = obj->GetInstanceScript())
            {
                ...
                case 0:
                    // Guid passing
                    if (Creature * l_Pride = l_Instance->instance->GetCreature(l_Instance->GetData64(eData::DATA_SHA_OF_PRIDE)))
                    {
            ...
            switch (events.ExecuteEvent())
            {
                ...
                case Events::EVENT_CORRUPTION_PRISON:
                {
                    ...
                    {               
                        ...
                        {
                            if (Creature * l_ShaOfPride = m_Instance->instance->GetCreature(m_Instance->GetData64(eData::DATA_SHA_OF_PRIDE)))
                            {
            ...
            if (m_Explosion <= diff)
            {
                ...
                else
                {
                    ...
                    for (auto itr : l_ListPlayers)
                    {
                        ...
                        {
                            if (Creature * l_ShaOfPride = l_Instance->instance->GetCreature(l_Instance->GetData64(eData::DATA_SHA_OF_PRIDE)))
                            {
